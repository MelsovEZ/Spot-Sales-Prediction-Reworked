...
export default function OrderHeader({
  ...
  return (
    <div className="space-y-6 mb-6">
      <Card className="bg-white/5 backdrop-blur-sm border-gray-200">
          ...
          <div className="flex items-start space-x-4">
            ...
            <div className="min-w-0 flex-1">
              <div className="flex items-center justify-between">
                <h2 className="text-lg font-medium truncate">{currentDate}</h2>
                <span className="ml-2 inline-flex items-center rounded-md bg-white/10 px-2.5 py-1 text-xs text-black/80">
                  18 Mnd
                </span>
              </div>
              <div className="mt-3 space-y-1">
                <p className="text-sm text-black/60">{orderType}</p>
                <div className="flex items-baseline space-x-2">
                  <span className="text-xs text-black/40">
                    Order count: #{orderDetails}
                  </span>
                </div>
                <p className="text-xs text-black/40">{dineInCode}</p>
              </div>
            </div>
          </div>
        </CardContent>
      ...
      <Card className="bg-white/5 backdrop-blur-sm border-gray-200">
        <CardContent className="p-0">
          <ScrollArea className="w-full whitespace-nowrap">
            ...
            <ScrollBar orientation="horizontal" className="bg-black/5" />
          </ScrollArea>
        </CardContent>
      </Card>
    </div>
